...
\begin{table*}
    ...
    \begin{center}
        ...
        \begin{tabularx}{\linewidth}{Xp{1.5cm}Xp{7cm}X} \toprule
            ...
            Mosaic data augmentation & \cite{BWL20} & データ拡張 & 
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
            ...
            Self-adversarial training (SAT) & \cite{BWL20} & データ拡張 & 
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
    ...
    \begin{center}
        ...
        \begin{tabularx}{\linewidth}{Xp{1.5cm}Xp{7cm}X} \toprule
            ...
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
                \item Pascal VOC 2007 に対して，当時の最高検出性能を達成しつつ，R-CNN よりも 24-102倍速い処理を達成．
            \end{itemize}
            \\

            Atrous spatial pyramid pooling (ASPP)
            & \cite{CPKMY17} & 学習性能の向上 & 
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
                \item 特徴抽出において解像度が縮小されないように upsampling フィルタを用いた畳み込み(atrous convolution / dilated convolution)と，SPPを組み合わせた， ASPP を提案した．
            \end{itemize}
            &
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
                \item PASCAL VOC-2012 semantic image segmentation task にて2016年当時の最高性能を達成．
            \end{itemize}
            \\
            ...
            Soft-NMS (non-maximum supression)
            & \cite{BSCD17} & 推論性能の向上 & 
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
            ...
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
                \item PAN構造において，backbone側からトップダウン経路をスキップしてもう一つのボトムアップ経路の同じレベルへのスキップ接続を設けること．
            \end{itemize}
            &
            \\

            Scale-wise Feature Aggregation Module (SFAM)
            & \cite{ZSWTCCL19} & 学習性能の向上 & 
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
                \item scale-wise に特徴量を連結し，適応的attentionメカニズムを通して，multi-level 特徴ピラミッドへ特徴量を集約する．
            \end{itemize}
            &
            \\
            ...
            \begin{itemize}
                \vspace{-0.7\baselineskip}
                \setlength{\leftskip}{-3mm}
                \item スケール毎に定められた重みで level re-weighting を実行し，異なるスケールの特徴マップの荷重和を計算して特徴マップを統合する．
            \end{itemize}
            &
            \\
            \bottomrule
